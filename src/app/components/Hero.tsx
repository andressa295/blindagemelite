    ...
    <section ref={heroRef} className="hero" data-state="init" style={{ opacity: 0 }}>
      ...
      <div className="container">
        <div className="mediaCol">
          <div className="deviceWrapper">
            ...
            <div className="combo">
              <Image src={deviceComboSrc} alt="Celular e relógio blindados" fill sizes="(max-width: 900px) 80vw, 44vw" style={{ objectFit: "contain" }} />
            </div>
          </div>
        </div>
        ...
        <div className="textCol">
          ...
          <p className="lead">
            ...
            camada invisível e ultra-resistente, mantendo seu aparelho com aparência
      ...
      <style jsx>{`
        ...
        @media (min-width: 900px) {
          ...
          .hero { padding-block: clamp(16px, 3.2vw, 36px); }
        }

        .sparks { position: absolute; inset: 0; overflow: hidden; z-index: 0; pointer-events: none; }
        .spark {
          position:absolute; bottom:-10%; left:calc(2% + 96% * var(--r, .5));
        ...
        .hero.play .watch {
          animation:
            watchEnter 1.2s cubic-bezier(0.25, 1, 0.3, 1) 0s forwards,
            watchExit 0.5s ease 4.5s forwards;
        }
        ...
        @keyframes watchExit {
          to{opacity:0;transform:translate(-8%,-6%) rotate(-8deg) scale(.96);}
        }

        .hero.play .phone {
          animation:
            phoneEnter 1.5s cubic-bezier(0.25, 1, 0.3, 1) 2s forwards,
            phoneHold 1.4s linear 3.5s forwards,
        ...
        @keyframes phoneEnter {
          ...
          100%{opacity:1;transform:translateY(0) rotate(0) scale(1.02);}
        }
        @keyframes phoneHold { to{opacity:1;transform:translateY(0) scale(1.02);} }
        @keyframes phoneExit {
          to{opacity:0;transform:translateY(-4%) rotate(-3deg) scale(1.0);}
        }

        .hero.play .combo {
          animation: comboDepthIn 1s cubic-bezier(.2,1,.3,1) 5.1s forwards;
          z-index: 2;
        }
        @keyframes comboDepthIn {
          0%{opacity:0;transform:scale(.6) translateY(4%);filter:blur(6px);}
          70%{opacity:1;transform:scale(1.12) translateY(-1%);filter:blur(0);}
          100%{opacity:1;transform:scale(1.08) translateY(0);}
        }

        .textCol { position: relative; opacity: 0; animation: textIn .8s ease 6s forwards; }
        ...
        .subtitle {
          ...
          animation: subAppear 0.7s ease 6.3s forwards;
        }
        ...
        .ctaRow { display:flex; gap:12px; opacity:0; animation: btnRise .8s ease 7s forwards; }
        @keyframes btnRise {
        ...
        .btnGhost:hover { background: rgba(255, 255, 255, 0.12); }
      `}</style>
    </section>
  );
}
